...
const deleteSubjects = async (req, res) => {
    ...
    if (!department || !subjectIds || !Array.isArray(subjectIds)) {
        return res.status(400).json({ error: 'Department and subjectIds array are required' });
    }
    ...
    try {
        ...
        res.status(200).json({ message: 'Subjects deleted successfully', result: result });
    } catch (error) {
...
const viewSubjects = async (req, res) => {
    ...
    try {
        ...
        if (subjects.length === 0) {
            return res.status(404).json({ message: 'No subjects found for the given department' });
        }
...
const updateSubject = async (req, res) => {
    ...
    if (!department || !subjectId) {
        return res.status(400).json({ error: 'Department and subjectId are required' });
    }
